import type { View, ViewElement } from '~/features/activity-layouts/lib/activityViewCatalog';
import {
...
} from '~/features/activity-visualizations/lib/activityVisualizationRegistry';
...
export function resolveActivityViewLoadingElements(view: View | null): ViewElement[] {
...
export function shouldShowWholeActivityViewLoading({
  view,
  elements,
  activityStore,
}: {
  ...
  if (!activityStore.loaded || !activityStore.query_options) {
    return true;
  }

  if (!activityStore.buckets.loaded) {
    return true;
  }

  return elements.some(element => !isActivityVisualizationReady(element.type, activityStore));
}
