...
import {
  ...
  PUBLIC_ACTIVITY_VISUALIZATIONS,
...
export function isSupportedActivityVisualization(type: string): boolean {
  return PUBLIC_ACTIVITY_VISUALIZATIONS.has(type);
}
...
export function shouldRefreshBucketsForQuery(query_options: QueryOptions): boolean {
  return Boolean(query_options.force) || shouldIncludeBrowserData(query_options);
}

export function shouldUseDashboardDtoFlow(query_options: QueryOptions): boolean {
