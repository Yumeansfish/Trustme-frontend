...
export default {
  ...
  methods: {
    ...
    async save() {
	      ...
	      await getClient().replaceEvent(this.bucket_id, this.editedEvent);
    ...
    async delete_() {
	      ...
	      await getClient().deleteEvent(this.bucket_id, this.event.id);
	      this.$emit('deleted', this.event);
	    },
	    async getEvent() {
	      if (this.bucket_id && this.event && this.event.id) {
	        this.editedEvent = await getClient().getEvent(this.bucket_id, this.event.id);
	      } else {
        this.editedEvent = null;
      }
    },
